...
import com.insoftbumdesku.passenger.activity.ChatActivity;
import com.insoftbumdesku.passenger.activity.MainActivity;
import com.insoftbumdesku.passenger.activity.ProgressActivity;
import com.insoftbumdesku.passenger.activity.SplashActivity;
import com.insoftbumdesku.passenger.constants.BaseApp;
import com.insoftbumdesku.passenger.json.fcm.DriverResponse;
...
//    public static final String BROADCAST_ACTION = "com.saw.passenger";
    public static final String BROADCAST_ACTION = "com.insoftbumdesku.passenger";
    ...
    private void messageHandler(RemoteMessage remoteMessage) {
        ...
        if (Objects.requireNonNull(remoteMessage.getData().get("type")).equals("3")) {
            if (user != null) {
        ...
        else if (Objects.requireNonNull(remoteMessage.getData().get("type")).equals("1")) {
            if (user != null) {
    ...
    private void orderHandler(RemoteMessage remoteMessage) {
        ...
        } else if (Objects.equals(remoteMessage.getData().get("response"), "11")) {
            playSound1();
            notificationSendAccepted(remoteMessage);
        }else if (Objects.equals(remoteMessage.getData().get("response"), "12")) {
            playSound1();
            notificationSendAccepted(remoteMessage);
        }else if (Objects.equals(remoteMessage.getData().get("response"), "13")) {
            playSound1();
            notificationSendAccepted(remoteMessage);
        }else if (Objects.equals(remoteMessage.getData().get("response"), "14")) {
            playSound1();
            notificationSendAccepted(remoteMessage);
        }
        else if (Objects.equals(remoteMessage.getData().get("response"), "15")) {
            playSound1();
            notificationSendAccepted(remoteMessage);
        }
    }
    ...
    private void notificationSendAccepted(RemoteMessage remoteMessage) {
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(getApplicationContext(), "notify_001");
        Intent intent1 = new Intent(getApplicationContext(), ProgressActivity.class);
        intent1.addFlags(FLAG_ACTIVITY_NEW_TASK);
        intent1.putExtra("id_transaksi", remoteMessage.getData().get("id_transaksi"));
        ...
        mBuilder.setContentIntent(pIntent1);
        mBuilder.setSmallIcon(R.drawable.logo);
        mBuilder.setContentTitle("Paket Telah Diterima");
        mBuilder.setContentText(remoteMessage.getData().get("desc"));
        mBuilder.setPriority(Notification.PRIORITY_MAX);
        mBuilder.setAutoCancel(true);
